...
BEGIN
  ...
  VALUES (
    NEW.id,
    -- Fallback to 'user_XYZ' if username is missing (OAuth)
    COALESCE(NEW.raw_user_meta_data->>'username', 'user_' || substr(NEW.id::text, 1, 8)),
    -- Fallback to 'name' claim or email if full_name is missing
    COALESCE(NEW.raw_user_meta_data->>'full_name', NEW.raw_user_meta_data->>'name', NEW.email),
    -- Fallback to DiceBear if avatar is missing
    COALESCE(NEW.raw_user_meta_data->>'avatar_url', 'https://api.dicebear.com/7.x/bottts/svg?seed=' || NEW.id),
    -- These will be NULL for OAuth initially, prompting the "Complete Profile" flow
...
-- Traces: Public Read (if visibility allows), Auth Create
CREATE POLICY "Traces are viewable by everyone" 
ON public.traces FOR SELECT USING (true); -- Simplified for MVP
